...
class HeaderComponent extends React.Component{
    ...
    componentWillMount(){
      request.get('https://tienda-963d5.firebaseio.com/productos.json')
              .end((err, res)=>{
                if(err || !res.ok){
        ...
        (function loop(i, shoppingCart, productosFiltrados) {
            if (i < shoppingCart.length){
                new Promise(resolve => {
                    ...
                    request.patch(`https://tienda-963d5.firebaseio.com/productos/${ index }.json`,JSON.stringify(data)).end((err,res)=>{
